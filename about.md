<img src="about.png" style="width: 100%; height: auto; border-radius: 12px; box-shadow: 0 4px 15px rgba(92, 83, 137, 0.3);">

# Zeo Neyo

<div class="profile-card">

probably insert twitch bio ngl 
> NIGHTMARE NIGHTMARE NIGHTMARE

---

🤍 Who is Zeo Neyo? 🖤
* Demi-God : something something.
* Lore stuff : yip yap yip yap
* Heathen's mentioned : lore lore

</div>

---

<style>
  /* 1. Fix the Blue Text for GitHub/Docsify */
  .profile-card, 
  .profile-card p, 
  .profile-card li, 
  .markdown-section,
  .markdown-section p,
  .markdown-section li {
    color: #f0efe4 !important; /* Creamy white */
    text-decoration: none !important;
  }

  /* 2. Style the Info Card */
  .profile-card {
    background: rgba(52, 54, 80, 0.7); /* Slightly darker for better contrast */
    padding: 25px;
    border-radius: 15px;
    border: 1px solid rgba(13, 2, 34, 0.3);
    margin: 20px 0;
  }

  /* 3. Heading Colors */
  .profile-card h3 {
    color: #efebb8 !important; /* Pale Gold */
    margin-top: 0;
  }

  /* 4. Center the Main Title */
  h1 {
    text-align: center;
    color: #f0efe4 !important;
    text-transform: uppercase;
    letter-spacing: 2px;
    margin-top: 20px;
  }
  
  /* Remove potential blue from list bullets */
  ul li::before {
    color: #efebb8 !important;
  }
</style>